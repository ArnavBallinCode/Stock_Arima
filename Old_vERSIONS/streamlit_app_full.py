...
def build_lstm_model(data, look_back=60):
    """Build and train LSTM model with proper TensorFlow configuration"""
    if not TENSORFLOW_AVAILABLE:
        st.error("TensorFlow is not available for LSTM modeling.")
        return None, None
    ...
    try:
        ...
        model = Sequential([
            LSTM(25, return_sequences=True, input_shape=(look_back, 1)),
            Dropout(0.1),
        ...
        with st.spinner("Training LSTM model..."):
            history = model.fit(X_train, y_train, batch_size=16, epochs=20, verbose=0)
